...
/* @var $searchModel frontend\models\qrcodeCodeSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = '二维码数据';//Yii::t('tbhome', 'QrcodeData');
...
<div class="row">
   ...
   /*      [
                ...
                $urlval=yii\helpers\Url::to(['qrcode/qrcodepage', 'code'=>$data->code, 'replyid'=>$data->replyid, 'productid'=>$data->replyid], true);
               ...
               'header'=>'网址',
                'buttons' => [
                    'view' => function ($url, $model, $key) {
                        $url=Url::to(['/qrcode/qrcode/qrcodepage', 'replyid'=>$model->replyid, 'productid'=>$model->productid, 'code'=>$model->code], true);
                       // $url=$model->url;
                        $options = [
...
<?=Html::a('导出全部数据', ['excelall'], ['class' => 'btn btn-danger'])?>
    <form method="post" id="login-form" role="form" action="<?=Url::to(['excelall'])?>" >
        <label>按序号导出数据：</label>
 ...
 <p>
    <form method="post" id="login-form" role="form" action="<?=Url::to(['genimage'])?>" >
