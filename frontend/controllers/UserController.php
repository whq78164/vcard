...
//return $this->redirect(['/user/user']);
        ...
        if ($info==null){
            $info=new Info();
            $info->uid=$uid;
            //        \Yii::$app->session->setFlash('error', '请完善个人信息！');
            //        return $this->redirect(['user/user']);
        }
        $request=Yii::$app->getRequest();

        if($request->isPost){
            if($baseuser->load($request->post())&&$baseuser->save()&&$info->load($request->post())&&$info->save()){
                Yii::$app->getSession()->setFlash('success', '名片信息设置成功！');
            }else{
    ...
    public function actionPassword()
    {
        ...
        if ($request->isPost) {
            ...
            if (!($model->validatePassword($oldpassword))) {
                ...
                Yii::$app->getSession()->setFlash('danger', '密码长度不得少于6位！');
                return $this->redirect(['setting']);
