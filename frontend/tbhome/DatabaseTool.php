...
class DatabaseTool
{
    ...
    public function __construct($config = array())
    {
        ...
        //        $this->handler = new \PDO(Yii::$app->db->dsn, Yii::$app->db->username, Yii::$app->db->password);
    ...
    private function writeToFile($tables = array(), $ddl = array(), $data = array())
    {
        ...
        $str = iconv("gb2312","utf-8", $str);//解决Windows中文乱码问题。
        echo file_put_contents($this->config['target'], $str) ? '<br/>数据库备份成功!<br/>备份文件：'.$this->config['target'].'<br/>花费时间' . (microtime(true) - $this->begin) . 'ms' : '备份失败!';
    }
    /**
     * 错误信息
     * @return mixed
